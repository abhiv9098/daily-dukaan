                  ...
                  <Card className="rounded-2xl border-none bg-white dark:bg-slate-900/40 p-3 hover:bg-slate-50 dark:hover:bg-slate-900/60 transition-colors cursor-pointer group shadow-sm">
                    <div className="flex items-center gap-3">
                      ...
                      <button
                        ...
                        className="inline-flex items-center gap-2 rounded-2xl border border-rose-200 bg-rose-50 px-3 py-2 text-xs font-bold text-rose-700 shadow-sm hover:bg-rose-100 transition-colors"
              ...
              {selectedCustomer && (
                <div className="mb-2">
                  ...
                  <div className="max-h-40 overflow-y-auto divide-y divide-slate-100">
                    {(creditTransactions || []).filter(ct => ct.customerId === selectedCustomer.id).slice(0, 12).map((ct) => (
                      <div key={ct.id} className="py-2 flex items-center justify-between">
                        <div className="text-sm">
                          <div className="font-medium text-slate-900">{ct.description || (ct.type === 'give' ? 'Udhaar Diya' : 'Udhaar Mila')}</div>
                          <div className="text-xs text-slate-500">{new Date(ct.date).toLocaleString()}</div>
                        </div>
                        <div className={cn("font-bold", ct.type === 'give' ? 'text-emerald-600' : 'text-rose-600')}>
                          {ct.type === 'give' ? '+' : '-'}{formatCurrency(ct.amount, settings.currency)}
                        </div>
                      </div>
                    ))}
                    {((creditTransactions || []).filter(ct => ct.customerId === selectedCustomer.id).length === 0) && (
                      <div className="py-2 text-xs text-slate-500">No transactions yet</div>
                    )}
                  </div>
                </div>
              )}
